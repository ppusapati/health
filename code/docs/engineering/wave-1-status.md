...
| SRS-NUR-014 | Transfusion monitoring linked to a blood-product episode; the reaction action runs from the bedside | **Implemented** — *by the blood bank since migration 0047.* The Wave-1 nursing record duplicated `bloodbank.episode` and was the one that could not answer a look-back, so it was migrated into it and the four nursing RPCs now refuse and name `BloodBankService`. The two rules this context held and that one did not are there now: a transfusion does not start without the observations it will be read against, and reporting a reaction stops the transfusion and records what was done at the bedside |
