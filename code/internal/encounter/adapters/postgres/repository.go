// Package postgres is the encounter context's persistence adapter.
//
// It is the only package permitted to issue SQL against the encounter schema
...
func scopeTenantID(scope authctx.TenantScope) (uuid.UUID, error) {
	if scope.IsZero() {
		return uuid.UUID{}, rpcerr.Internal("ENC_NO_TENANT_SCOPE",
			"a repository call needs a verified tenant scope")
	}
	parsed, err := uuid.Parse(scope.TenantID())
	if err != nil {
		return uuid.UUID{}, rpcerr.Internal("ENC_TENANT_ID_INVALID",
...
func notFound() error {
	return rpcerr.NotFound("ENC_NOT_FOUND", "no such encounter record")
}
